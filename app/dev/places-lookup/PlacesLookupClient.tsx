    ...
    <div className="flex items-center gap-3">
      ...
      {state.kind === "unauthenticated" && (
        <span className="text-sm text-red">
          Your session expired — log in again to save.
        </span>
      )}
      {state.kind === "error" && (
        <span className="text-sm text-red">{state.message}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-6">
      <Card className="p-5">
        ...
        />
        <Field label="Primary category" value={place.primaryCategory} />
        <Field
          label="Categories"
        ...
        <div className="mt-4 border-t border-paper-line pt-4">
          <SaveControl place={place} isLoggedIn={isLoggedIn} />
        </div>
      </Card>
...
export function PlacesLookupClient({ isLoggedIn }: { isLoggedIn: boolean }) {
      ...
      <div>
        ...
        {!isLoggedIn && (
          <p className="mt-2 text-sm text-ink-soft">
            You&apos;re not logged in — lookups still work, but you won&apos;t be
            able to save results.{" "}
            <Link href="/login" className="font-medium text-brass hover:underline">
              Log in
            </Link>{" "}
            or{" "}
